...
exports.getUserById = exports.editUser = exports.addUser = exports.getAllSessions = exports.getAdminById = exports.assignMentorToMentee = exports.assignMentor = exports.getSessionStats = exports.getAllMentorshipMatches = exports.getTotalSessionsHeld = exports.getAllUsers = void 0;
...
const assignMentor = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const userCheck = yield db_config_1.pool.query(`SELECT id, role FROM users WHERE id = ANY($1::uuid[])`, [[mentorId, menteeId]]);
        console.log("User check result:", userCheck.rows);
        console.log("Reducing user roles");
        const roles = userCheck.rows.reduce((acc, user) => (Object.assign(Object.assign({}, acc), { [user.id]: user.role })), {});
        ...
        const exists = yield db_config_1.pool.query(`SELECT * FROM mentorship_match WHERE "mentorId" = $1 AND "menteeId" = $2`, [mentorId, menteeId]);
        ...
        const insert = yield db_config_1.pool.query(`INSERT INTO mentorship_match ("mentorId", "menteeId")
       VALUES ($1, $2)
       RETURNING *;`, [mentorId, menteeId]);
...
const assignMentorToMentee = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const adminResult = yield db_config_1.pool.query(`SELECT username FROM admins WHERE "userId" = $1`, [adminId]);
        console.log("Admin query result:", (_a = adminResult.rows[0]) === null || _a === void 0 ? void 0 : _a.username);
        if (adminResult.rowCount === 0) {
            console.log("Admin not found in admins table");
            yield db_config_1.pool.query("ROLLBACK");
        ...
        const adminUserResult = yield db_config_1.pool.query(`SELECT id FROM users WHERE username = $1 AND role = 'admin'`, [adminUsername]);
        ...
        const mentorResult = yield db_config_1.pool.query(`SELECT username FROM mentors WHERE "userId" = $1`, [mentorId]);
        console.log("Mentor query result:", (_b = mentorResult.rows[0]) === null || _b === void 0 ? void 0 : _b.username);
        ...
        const mentorUserResult = yield db_config_1.pool.query(`SELECT id FROM users WHERE username = $1 AND role = 'mentor'`, [mentorUsername]);
        ...
        const menteeResult = yield db_config_1.pool.query(`SELECT username FROM mentees WHERE "userId" = $1`, [menteeId]);
        console.log("Mentee query result:", (_c = menteeResult.rows[0]) === null || _c === void 0 ? void 0 : _c.username);
        ...
        const menteeUserResult = yield db_config_1.pool.query(`SELECT id FROM users WHERE username = $1 AND role = 'mentee'`, [menteeUsername]);
        ...
        const existing = yield db_config_1.pool.query(`SELECT 1 FROM mentorship_match WHERE "menteeId" = $1`, [resolvedMenteeUserId]);
        console.log("Existing match result:", existing.rows);
        if (((_d = existing.rowCount) !== null && _d !== void 0 ? _d : 0) > 0) {
        ...
        console.log("Inserting new mentorship match");
        const insert = yield db_config_1.pool.query(`INSERT INTO mentorship_match ("mentorId", "menteeId", "adminId", id)
       VALUES ($1, $2, $3, uuid_generate_v4())
       RETURNING *`, [resolvedMentorUserId, resolvedMenteeUserId, resolvedAdminUserId]);
        console.log("Insert result:", insert.rows);
        console.log("Committing transaction");
        yield db_config_1.pool.query("COMMIT");
...
const getAdminById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const id = (_a = req.user) === null || _a === void 0 ? void 0 : _a.adminId;
    ...
    `;
        console.log("Query:", query);
        console.log("Executing query");
        const { rows } = yield db_config_1.pool.query(query, [id]);
        console.log("Query result:", rows);
        if (rows.length === 0) {
...
const addUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const userExists = yield db_config_1.pool.query(`SELECT * FROM users WHERE username = $1 OR email = $2`, [username, email]);
        console.log("User exists query result:", userExists.rows);
        if (((_a = userExists.rowCount) !== null && _a !== void 0 ? _a : 0) > 0) {
        ...
        const { rows } = yield db_config_1.pool.query(`INSERT INTO users (id, username, email, "passwordHash", role)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, username, email, role`, [userId, username, email, passwordHash, role]);
        ...
        if (role === "mentor") {
            ...
            const mentorResult = yield db_config_1.pool.query(`INSERT INTO mentors ("mentorId", "userId", username,"shortBio", goals) 
         VALUES ($1, $2, $3, $4, $5) RETURNING "mentorId", "userId","shortBio", goals`, [roleId, userId, username, shortBio, goals]);
            console.log("Inserted mentor:", mentorResult.rows[0]);
        }
        else if (role === "mentee") {
            console.log("Inserting into mentees table");
            const menteeResult = yield db_config_1.pool.query(`INSERT INTO mentees ("menteeId", "userId", username, "shortBio", goals) 
         VALUES ($1, $2, $3, $4, $5) RETURNING "menteeId", "userId","shortBio", goals`, [roleId, userId, username, shortBio, goals]);
            console.log("Inserted mentee:", menteeResult.rows[0]);
        }
        else if (role === "admin") {
            console.log("Inserting into admins table");
            const adminResult = yield db_config_1.pool.query(`INSERT INTO admins ("adminId", "userId", username, "shortBio", goals) 
         VALUES ($1, $2, $3, $4, $5) RETURNING "adminId", "userId", "shortBio", goals`, [roleId, userId, username, shortBio, goals]);
...
const editUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const existing = yield db_config_1.pool.query(`SELECT * FROM users WHERE id = $1`, [
            id,
        ]);
...
const getUserById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { rows } = yield db_config_1.pool.query(`SELECT * FROM users WHERE "id" = $1`, [
            id,
        ]);
        console.log("Query result:", rows);
        console.log("Checking if user exists");
