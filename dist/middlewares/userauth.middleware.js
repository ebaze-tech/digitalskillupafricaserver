...
const authenticateUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (user.role === 'mentor') {
            const result = yield db_config_1.pool.query(`SELECT "mentorId" FROM mentors WHERE "userId" = $1`, [user.roleId]);
            if (result.rows.length > 0) {
                user.roleId = result.rows[0].mentorId;
            }
        }
        else if (user.role === 'mentee') {
            const result = yield db_config_1.pool.query(`SELECT "menteeId" FROM mentees WHERE "userId" = $1`, [user.id]);
            if (result.rows.length > 0) {
                user.roleId = result.rows[0].menteeId;
            }
        }
        else if (user.role === 'admin') {
            const result = yield db_config_1.pool.query(`SELECT "adminId" FROM admins WHERE "userId" = $1`, [user.id]);
            if (result.rows.length > 0) {
                user.roleId = result.rows[0].adminId;
            }
        }
        req.user = user;
        next();
